...
const ChapterDetail = () => {
    ...
    return (
        ...
        <div className="container mx-auto p-6 flex items-center justify-between h-screen gap-8">
            {/* Left Sidebar - Title and Back Button */}
            <div className="flex flex-col justify-between h-full py-8 w-64 flex-shrink-0">
                {/* Swapped: Back button is now on top */}
                <GazeButton
                    whileHover={{ scale: 1.1 }}
                    whileTap={{ scale: 0.95 }}
                    onClick={() => router.back()}
                    className="bg-[#E64A4A] text-[#293041] px-6 py-3 rounded-lg hover:bg-[#3f4759] hover:text-[#dbe2f9] shadow-lg w-48 h-24"
                >
                    Back
                </GazeButton>

                <motion.h1
                    className="text-3xl font-bold text-[#E64A4A] mb-8"
            ...
            {/* Center Content - Navigation and Images */}
            <div className="flex items-center justify-center gap-6 flex-1">
                <GazeButton
                    ...
                    className={`p-10 rounded-full bg-[#E64A4A] text-white text-5xl shadow-lg transform transition-transform duration-300 hover:scale-110 flex-shrink-0 ${
                        currentPage === 0
                ...
                <motion.div
                    className="grid gap-4 md:gap-8 sm:grid-cols-2 md:grid-cols-2 flex-shrink-0"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                        ...
                        .map((img) => (
                            ...
                            >
                                <LazyImage
                                    src={img.url}
                                    alt={`Page ${img.id}`}
                                    className="max-h-[85vh] max-w-[32vw] shadow-lg rounded-lg object-contain"
                                />
                            </motion.div>
                        ))}
                </motion.div>
